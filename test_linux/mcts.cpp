...
void init_processor() 
{
    ...
    //The servers from index 1 to global.n_processors - global.n_MBS_processors - 1 are SBSs
    ...
    //The remaining servers are MBSs
...
Node tree_policy(DAG dag, Tree tree, Queue queue) {
    ...
    while (node->children != NULL) {// Traverses all nodes
        ...
        update_scheduled_task(task, child->processor, fp_process);// Update the task status, recording the start time and completion time
...
double get_speedup(DAG dag, FILE* fp) {
   ...
   for (int i = 0; i < dag->nb_levels; i++)
    ...
    {
        update_scheduled_task(dag->levels[i][j], best_performance_processor, fp);
    }
...
void UCTsearch(DAG dag, Tree tree, Queue queue) {
        ...
        result.mcts_speedup = get_speedup(dag, fp_process) / result.mcts_makespan;
        free(queue);
        free(tree->root);
        free(tree);
        free(task_ready);
